...
- **DeepSignal-CyclePlan-4B-V2** —— CyclePlan 后续版本更新，进一步强化可执行配时输出、响应速度与排队/延误表现
...
DeepSignal-CyclePlan-4B-V2 是 CyclePlan 系列的后续版本更新，适用于 `llama.cpp`、LM Studio 等 GGUF 兼容本地推理环境。它延续 V1 的周期级控制目标：输入某个路口各相位的预测交通状态与绿灯约束后，输出下一信号周期中每个相位的最终绿灯时长。
...
- **2026-07-01**：发布 **DeepSignal-CyclePlan-4B-V2**。
...
**结论**：在 `300-900s` 早期拥堵窗口中，**DeepSignal-CyclePlan-4B-V2** 在三项交通运行指标上均优于 V1。在所有评估模型中，V2 取得最低的目标路口平均等待时间（`61.43s`）、平均排队车辆数（`15.54`）和每辆车平均延误（`112.11s`），V1 的平均响应时间最低（`2.73s`）。
...
DeepSignal-CyclePlan-4B-V2 的 GGUF 文件通过 Hugging Face 分发：
