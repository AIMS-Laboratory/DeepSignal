...
该对比统一使用 `300-900s` 统计窗口，模型温度为 `0.2`，用于观察早期车辆等待表现、排队水平、旅行时间和响应速度。
...
令 `V_target` 表示经过目标路口相关路段、在评估窗口内出发并完成行程的车辆集合。`w_i` 表示车辆 `i` 在 SUMO `tripinfo` 中记录的累计等待时间，`tau_i = a_i - d_i` 表示车辆完成行程的旅行时间。
