...
该对比统一使用 `300-900s` 统计窗口，模型温度为 `0.2`，用于观察早期车辆等待表现、排队水平、旅行时间和控制输出稳定性。

本节额外使用以下指标：

- **目标路口平均等待时间**：经过目标路口相关车辆的真实平均等待时间，来自 SUMO `tripinfo` 车辆记录，单位秒，越低越好。
- **全路网平均等待时间**：全路网完成车辆的真实平均等待时间，来自 SUMO `tripinfo` 车辆记录，单位秒，越低越好。
- **目标路口平均旅行时间**：经过目标路口相关车辆的平均旅行时间，来自完成行程的 `duration`，单位秒，越低越好。
- **控制可用率**：模型输出能被解析并通过配时约束检查、可作为控制方案使用的比例，越高越好。

令 `V_target` 表示经过目标路口相关路段、在评估窗口内出发并完成行程的车辆集合，`V_network` 表示全路网内满足同一窗口条件并完成行程的车辆集合。`w_i` 表示车辆 `i` 在 SUMO `tripinfo` 中记录的累计等待时间，`tau_i = a_i - d_i` 表示车辆完成行程的旅行时间。
...
| 模型 | 温度 | 目标路口平均等待时间 (s) | 全路网平均等待时间 (s) | 目标路口平均旅行时间 (s) | 平均排队车辆数 | 每辆车平均延误 (s) | 控制可用率 | 平均响应时间 (s) |
|:---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
`**`：该表统一使用 `300-900s` 窗口。`目标路口平均等待时间`、`目标路口平均旅行时间` 与 `全路网平均等待时间` 根据 SUMO `tripinfo` 中 `depart` 落在窗口内且完成行程的车辆统计；`平均排队车辆数` 与 `每辆车平均延误` 用于补充反映窗口内拥堵水平与车辆延误。
