...
DeepSignal 是我们自主微调的交通信号控制大模型套件。当前发布版本包括三个模型：

- **DeepSignal-Phase-4B-V1** —— 下一信号相位预测（预测下一个激活的相位及其持续时间）
- **DeepSignal-CyclePlan-4B-V1** —— 信号周期配时优化（输出下一周期各相位的绿灯时间分配）
- **DeepSignal-CyclePlan-4B-V1.2** —— CyclePlan 后续版本更新，进一步强化可执行配时输出、响应速度与排队/延误表现

模型均可在 Hugging Face 获取：
...
**输入格式**：JSON 包装在 `【cycle_predict_input_json】...【/cycle_predict_input_json】` 标签内，包含各相位的预测饱和度、最小/最大绿灯时间约束和容量信息。其中 `pred_saturation = pred_wait / capacity`，`pred_wait` 为预测的下一周期该相位的等待车辆数，可通过预测模型（如 LSTM、TCN）基于历史交通数据进行预测。

**输出格式**：一个 JSON 对象数组 `[{“phase_id”: <int>, “final”: <int>}, ...]`，为每个相位分配最终的绿灯时间（秒）。

## DeepSignal-CyclePlan-4B-V1.2

DeepSignal-CyclePlan-4B-V1.2 是 CyclePlan 系列的后续版本更新，适用于 `llama.cpp`、LM Studio 等 GGUF 兼容本地推理环境。它延续 V1 的周期级控制目标：输入某个路口各相位的预测交通状态与绿灯约束后，输出下一信号周期中每个相位的最终绿灯时长。

该版本基于 Qwen3 4B 架构，主要用于 SUMO 仿真、交通信号配时研究和本地控制器原型验证。评测时使用 DeepSignal 风格 prompt：模型先给出极简推理，再将最终 JSON 配时方案放在 `<SOLUTION>...</SOLUTION>` 内。
...
**输入格式**：JSON 包装在 `【cycle_predict_input_json】...【/cycle_predict_input_json】` 标签内，包含 `prediction.phase_waits` 下的 `phase_id`、`pred_wait`、`pred_saturation`、`min_green`、`max_green` 和 `capacity`。其中 `pred_saturation = pred_wait / capacity`。

**输出格式**：先输出推理块，再在 `<SOLUTION>...</SOLUTION>` 内输出 JSON 对象，例如 `<SOLUTION>{"1": 55, "2": 30}</SOLUTION>`；其中键为相位 ID 字符串，值为该相位最终绿灯时长的整数秒。
...
- **2026-07-01**：发布 **DeepSignal-CyclePlan-4B-V1.2**。
...
### CyclePlan 模型评估对比（300-900s）$^{**}$

我们在 SUMO 仿真平台中对 CyclePlan 模型进行闭环控制评估。模型在每个决策周期接收当前路口各相位的预测等待车辆数、预测饱和度以及最小/最大绿灯约束，并输出下一周期各相位的最终绿灯时长。仿真平台随后将该配时方案应用到 SUMO，并统计交通运行指标与模型执行指标。所有评估均在 **NVIDIA GeForce RTX 5090 GPU** 上进行。

该对比统一使用 `300-900s` 统计窗口，模型温度为 `0.2`，用于观察早期车辆等待表现、排队水平、车辆延误和响应速度。

本节额外使用目标路口平均等待时间指标：

- **目标路口平均等待时间**：经过目标路口相关车辆的真实平均等待时间，来自 SUMO `tripinfo` 车辆记录，单位秒，越低越好。

令 `V_target` 表示经过目标路口相关路段、在评估窗口内出发并完成行程的车辆集合。`w_i` 表示车辆 `i` 在 SUMO `tripinfo` 中记录的累计等待时间。

$$
\mathrm{TargetAWT}=\frac{\sum_{i \in \mathcal{V}_{target}} w_i}{|\mathcal{V}_{target}|}
$$

| 模型 | 温度 | 目标路口平均等待时间 (s) | 平均排队车辆数 | 每辆车平均延误 (s) | 平均响应时间 (s) |
|:---:|---:|---:|---:|---:|---:|
...
| Gemma3-12B-IT | 0.2 | 82.11 | 18.30 | 118.43 | 31.22 |
| Qwen3-4B | 0.2 | 98.10 | 19.93 | 129.70 | 19.16 |
| GPT-OSS-20B | 0.2 | 92.53 | 18.78 | 123.80 | 35.58 |

`**`：该表统一使用 `300-900s` 窗口。`目标路口平均等待时间`根据 SUMO `tripinfo` 中 `depart` 落在窗口内且完成行程的车辆统计；`平均排队车辆数`与`每辆车平均延误`用于补充反映窗口内拥堵水平与车辆延误。`平均响应时间`测试不限制思考链或最终回复的最大 token 数。

**结论**：在 `300-900s` 早期拥堵窗口中，**DeepSignal-CyclePlan-4B-V1.2** 在三项交通运行指标上均优于 V1。在所有评估模型中，V1.2 取得最低的目标路口平均等待时间（`61.43s`）、平均排队车辆数（`15.54`）和每辆车平均延误（`112.11s`），V1 的平均响应时间最低（`2.73s`）。

![CyclePlan 模型评估对比](images/cycleplan_model_comparison_v1_2_zh.png)
...
CyclePlan 模型同样提供 GGUF 格式，包含 **F16**（全精度）和 **Q4_K_M**（量化）两个版本，可在 Hugging Face 模型库获取。
...
DeepSignal-CyclePlan-4B-V1.2 的 GGUF 文件通过 Hugging Face 分发。下载仓库名与文件名暂时保留原 `V2` 标识，以保持下载兼容：
