...
- prediction.phase_waits[*].pred_saturation:预测饱和度(pred_wait / capacity)。其中 pred_wait 为预测的下一周期该相位的等待车辆数，可通过时序预测模型（如 [LightGBM](https://github.com/microsoft/LightGBM)）基于历史交通数据进行预测。
