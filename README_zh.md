...
**输入格式**：JSON 包装在 `【cycle_predict_input_json】...【/cycle_predict_input_json】` 标签内，包含各相位的预测饱和度、最小/最大绿灯时间约束和容量信息。其中 `pred_saturation = pred_wait / capacity`，`pred_wait` 为预测的下一周期该相位的等待车辆数，可通过预测模型（如 [LightGBM](https://github.com/microsoft/LightGBM)）基于历史交通数据进行预测。
